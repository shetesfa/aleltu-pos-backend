...
// 1. Low stock alerts
$res = mysqli_query($conn, "SELECT si.item_name, si.current_stock, si.unit, COALESCE(si.low_stock_alert,5) as threshold 
    FROM seller_inventory si WHERE si.branch_id=$branch_id AND si.current_stock <= COALESCE(si.low_stock_alert,5) ORDER BY si.current_stock ASC LIMIT 20");
if ($res) {
    while ($r = mysqli_fetch_assoc($res)) {
        $alerts[] = ['type'=>'LOW_STOCK','level'=>$r['current_stock']<=0?'CRITICAL':'WARNING',
            'title'=>'Low Stock: '.$r['item_name'],
            'message'=>'Only '.$r['current_stock'].' '.$r['unit'].' remaining (threshold: '.$r['threshold'].')',
            'data'=>$r];
    }
}
...
$res5 = mysqli_query($conn, "SELECT si.item_name, si.current_stock FROM seller_inventory si 
    WHERE si.branch_id=$branch_id AND si.current_stock <= 0 LIMIT 10");
if ($res5) {
